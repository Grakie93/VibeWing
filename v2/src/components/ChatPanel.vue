...
<template>
  <section class="chat-panel">
    <div class="chat-head"><select v-model="activeId" class="chat-select"><option v-for="chat in chats" :key="chat.id" :value="chat.id">{{chat.title}}</option></select><button type="button" @click="newChat">＋ 新对话</button></div>
    <div class="chat-messages"><template v-if="active"><article v-for="(message,index) in active.messages" :key="index" :class="['chat-message',message.role]"><strong>{{message.role==='user'?'我':'AI'}}</strong><p>{{message.content}}</p></article></template><p v-if="busy" class="thinking">思考中…</p><p v-if="!modelOptions.length" class="hint">请先在设置中添加模型平台并配置 API Key。</p></div>
    <form class="chat-composer" @submit.prevent="send"><textarea v-model="input" rows="2" placeholder="输入问题，Enter 发送；Shift+Enter 换行" @keydown="onKeydown"/><select v-model="selectedModel" class="model-select"><option v-for="model in modelOptions" :key="model.provider+model.id" :value="model.id">{{model.label}}</option></select><button class="send-button primary" :disabled="busy||!selected">➤</button></form>
