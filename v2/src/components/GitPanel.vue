...
<template>
  <section class="git-panel">
    ...
    <div v-else class="git-files"><label v-for="file in files" :key="file.path" class="git-file"><input v-model="selected" type="checkbox" :value="file.path" :disabled="file.staged && !file.unstaged" /><span :class="['git-status', statusClass(file)]">{{ statusLabel(file) }}</span><code>{{ file.path }}</code></label></div>
    <div class="git-stage-row"><span>{{ selected.length }} 个文件已选择</span><button type="button" :disabled="busy || !selected.length" @click="stage">暂存所选</button></div>
    <label class="commit-message">提交信息<textarea v-model="message" rows="3" placeholder="例如：feat: improve project controls" /></label>
    <div class="git-actions"><button type="button" :disabled="busy || !message.trim()" @click="commit">{{ busy ? '处理中…' : '提交暂存' }}</button><button type="button" class="primary" :disabled="busy" @click="push">推送远端</button></div>
  </section>
</template>
